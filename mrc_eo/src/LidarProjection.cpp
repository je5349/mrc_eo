...
LidarProjection::LidarProjection(ros::NodeHandle nh)
...
void LidarProjection::init() {
  ...
  sync->registerCallback(boost::bind(&LidarProjection::callback, this, _1, _2));

  pub = nodeHandle_.advertise<sensor_msgs::Image> (lidarProjectionTopicName, lidarProjectionImageQueueSize, lidarProjectionImageLatch);
}

int LidarProjection::normalize_data(double val, int min, int max, int scale)
{
  return ((max - val) / (max - min))*scale;
}

void LidarProjection::callback(const sensor_msgs::PointCloud2ConstPtr& cloud_msg, const sensor_msgs::ImageConstPtr& image_msg) {
  ...
  cvtColor(overlay, result, cv::COLOR_HSV2BGR);

  sensor_msgs::ImagePtr overlay_img_msg = cv_bridge::CvImage(std_msgs::Header(), "bgr8", result).toImageMsg();

  pub.publish(overlay_img_msg);
}
